...
class BaseLightGBMModel:
    ...
    def _predict(self, value, kind):
        ...
        return result.tolist() if hasattr(result, "tolist") else float(result)
    def transform(self, dataset, prediction_col=None, raw_prediction_col=None, probability_col=None, leaf_prediction_col=None):
        ...
        if probability:
            ...
            if prob_name: out = out.withColumn(prob_name, udf(lambda x: self._predict(x, "prob"), ArrayType(DoubleType()))(self.estimator.features_col))
        leaf_name = leaf_prediction_col or self.estimator.leaf_prediction_col
        if leaf_name: out = out.withColumn(leaf_name, udf(lambda x: self._predict(x, "leaf"), ArrayType(DoubleType()))(self.estimator.features_col))
        return out
